...
//        require(msg.sender == cryptogsAddress, "sender should be address of cryptogsAddress");

        //make sure this commit is unique / doesn't already exist
        require(commitReceipt[_commit][_sender] == 0, "make sure this commit is unique / doesn't already exist");

        //make sure there aren't already two stacks submitted
        require(stacksTransferred[_commit]<2, "make sure there aren't already two stacks submitted");
        stacksTransferred[_commit]++;

        //make sure this contract now owns these tokens
        NFTInterface cryptogsContract = NFTInterface(cryptogsAddress);
        require(cryptogsContract.tokenIndexToOwner(_token1)==address(this), "token1");
        require(cryptogsContract.tokenIndexToOwner(_token2)==address(this), "token2");
        require(cryptogsContract.tokenIndexToOwner(_token3)==address(this), "token3");
        require(cryptogsContract.tokenIndexToOwner(_token4)==address(this), "token4");
        require(cryptogsContract.tokenIndexToOwner(_token5)==address(this), "token5");
    ...
    function _flip(bytes32 _commit,uint8 round,NFTInterface cryptogsContract,uint[10] _tokens,uint8 tokenIndex,address _opponent,bool whosTurn) internal {
        address flipper;
        if(whosTurn) {
            flipper=msg.sender;
        }else{
